...
class CertificateTemplateController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $layoutJson = json_encode($request->layout_json, JSON_PRETTY_PRINT);
    ...
    public function update(Request $request, CertificateTemplate $certificateTemplate)
{
    ...
    DB::transaction(function () use ($request, $certificateTemplate) {

        // ⬇️ WAJIB SAMA DENGAN STORE
        ...
        if ($request->hasFile('image_template')) {
            ...
            if ($certificateTemplate->image_template
                && Storage::disk('public')->exists($certificateTemplate->image_template)) {
                Storage::disk('public')->delete($certificateTemplate->image_template);
            }

            $data['image_template'] = $request
                ->file('image_template')
                ->store('certificate/templates', 'public');
        }
    ...
    { 
        ...
        if ($certificateTemplate->image_template) {
            $path = str_replace('storage/', '', $certificateTemplate->image_template);
